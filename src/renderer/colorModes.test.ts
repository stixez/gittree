...
import { hashString, authorColor, hexLerp, nodeColor, colorCtxFromNodes, AUTHOR_COLORS } from './colorModes'
...
function node(p: Partial<SceneNode>): SceneNode {
  return {
    ...
    significant: false, message: '', authorName: 'Ada', authorTs: 100, parents: [],
    ...p,
  }
}
...
describe('nodeColor', () => {
  ...
  it('author mode is stable per author and varies across authors', () => {
    expect(nodeColor(node({ authorName: 'Ada' }), 'author', ctx)).toBe(authorColor('Ada'))
  })
